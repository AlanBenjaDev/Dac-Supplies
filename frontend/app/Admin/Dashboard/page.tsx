...
interface DashboardData {
  ...
  fecha_envio: string;
  pedido_id: number;
  nombre_producto: string; // Coincide con tu SQL
...
export default function AdminDashboard() {
  ...
  const filteredEnvios = data.filter(e =>
    ...
    e.nombre_producto.toLowerCase().includes(search.toLowerCase()) ||
        ...
        <div className="bg-white border border-gray-100 rounded-sm overflow-hidden shadow-[0_20px_60px_rgba(0,0,0,0.03)]">
          <div className="p-8 border-b border-gray-50 flex flex-col md:flex-row justify-between items-center gap-6">
            ...
            <div className="relative w-full md:w-96">
              ...
              <input 
                type="text" 
                placeholder="Buscar por ID, producto o apellido..." 
              ...
              <table className="w-full text-left border-collapse">
                <thead>
                  <tr className="text-[10px] uppercase tracking-[0.2em] text-gray-400 bg-gray-50/50">
                    <th className="px-8 py-5 font-black">Cliente & Documento</th>
                    <th className="px-8 py-5 font-black">Producto & Precio</th>
                    <th className="px-8 py-5 font-black">Ubicación & Contacto</th>
                    <th className="px-8 py-5 font-black">Dirección</th>
                ...
                <tbody className="divide-y divide-gray-50">
                  {filteredEnvios.map((envio, idx) => (
                    <tr key={`${envio.envio_id}-${idx}`} className="hover:bg-gray-50/50 transition-colors group">
                      <td className="px-8 py-6">
                        <div className="flex flex-col">
                          <span className="font-black text-black text-sm tracking-tighter uppercase">{envio.nombre} {envio.apellido}</span>
                          <span className="text-[9px] text-gray-400 uppercase font-bold tracking-widest">DNI: {envio.documento}</span>
                        </div>
                      </td>
                      <td className="px-8 py-6">
                        <div className="flex flex-col">
                      ...
                      <td className="px-8 py-6">
                        <div className="flex flex-col text-[11px]">
                          ...
                          <span className="text-gray-400 text-[9px] uppercase tracking-widest font-medium">CP: {envio.codigo_postal}</span>
                        </div>
                      </td>
                      <td className="px-8 py-6">
